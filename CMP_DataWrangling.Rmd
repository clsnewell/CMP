...
write.csv(RawIsotopeData, "Data/RawIsotopeData_04272022.csv")
...
Raw_S<- bind_rows(CMPatty_S, CMPBuffPants_S, CMPChoppedLiver_S, CMPeace_S, CMPeachment_S, CMPISeeDeadCMPeople_S, CMPlants_S, CMPlucheae_S, CMPLiverItUp_S, CMPMudBugs_S, CMPLiverLaughLov_S, CMPLiverOfMine_S, CMPMuscleMeat_S, CMPoaceae_S, CMPNotFood_S, CMPSampler_S, CMPSchwarzeneger_S, CMPScuds_S, CMPTunaOfTheFarm_S, CMPZootopia_S)
...
write.csv(Raw_S, "Data/Raw_S_04272022.csv")

#Manually added taxon columns, so read-in the excel versions of the raw data with taxon columns
RawIsotopeData_04272022 <- read_excel("Data/RawIsotopeData_04272022.xlsx")

Raw_S_04272022<- read_excel("Data/Raw_S_04272022.xlsx")
...
FishIsotopeData_S<- Raw_S_04272022 %>% filter(Taxon == "FISH")
...
write.csv(FishMuscleData, "Data/FishMuscleData_042722.csv")

write.csv(FishLiverData., "Data/FishLiverData_042722.csv")
```

## Let's do plants next.
```{r}
RawIsotopeData_04272022

PlantIsotopeData<- RawIsotopeData_04272022 %>% filter(Taxon == "PLANT")
...
PlantIsotopeData$SampleID<-gsub("_", "-", PlantIsotopeData$SampleID)
...
PlantIsotopeAndMetadata<- PlantIsotopeAndMetadata %>% unite("SampleID", c("SpecimenID","Trial"), sep="_", remove=FALSE)
colnames(PlantIsotopeAndMetadata)
#Okay so each plant does not have a row for sulfur and CN together. The S and CN data is separated.
#PlantIsotopeAndMetadata

PlantIsotopeAndMetadata2<- PlantIsotopeAndMetadata %>% group_by(SampleID, SpecimenID, Project, Trial, Taxon, Sample_ID, Bag_ID, SpeciesCode, Site, CollectionDate, Season, LabDate, Parts_of_plant_included_in_sample_for_drying, 'TapWaterExposure?', Notes, Initials, MaterialsGrounded, GrindingInitials, Class, Species) %>% summarise_each(funs(first(na.omit(.))))
  
#summarize('D13CVPDB(permill)' = sum('D13CVPDB(permill)', na.rm=TRUE), 'TotalC(ug)' = sum(), C_Comment = sum(C_Comment, na.rm=TRUE), 'D15NAir(permill)', 'TotalN(ug)', NComment, 'D15N_MeanReferenceSD_plusminus(permill)', 'D15N_MeanAbsAccuracy_plusminus(permill)', 'D13C_MeanReferenceSD_plusminus(permill)', 'D13C_MeanAbsAccuracy_plusminus(permill)', PlantNumber, Treatment, 'D34SVCDT(permill)', 'TotalS(ug)', S_Comment, 'D34S_MeanReferenceSD_plusminus(permill)', 'D34S_MeanAbsAccuracy_plusminus(permill)')

#SAVE IT
write.csv(PlantIsotopeAndMetadata, "Data/PlantIsotopeAndMetaData_042722.csv")
```

## Now zoops
```{r}
ZoopIsotopeData<- RawIsotopeData_04272022 %>% filter(Taxon == "ZOOP")
...
write.csv(ZoopIsotopeAndMetadata, "Data/ZoopIsotopeAndMetaData_042722.csv")
```
...
```{r}
InvertIsotopeData<- RawIsotopeData_04272022 %>% filter(Taxon == "INVERT")
InvertIsotopeData$SampleID<-gsub("_", "-", InvertIsotopeData$SampleID)
InvertIsotopeData_SampleIDs<- separate(InvertIsotopeData, col=SampleID, into=c('Project', 'Invertebrate', 'SpecimenID', 'Trial'), sep='-') #The sulfur data has _ instead of -.
...
#Same issue again. Sulfur and CN are separate but should be in the same row.
write.csv(LargeMacroinvert_IsotopeAndMetadata, "Data/LargeMacroinvert_IsotopeAndMetaData_042722.csv")
...
write.csv(SmallMacroinvert_IsotopeAndMetadata, "Data/SmallMacroinvert_IsotopeAndMetaData_042722.csv")
```
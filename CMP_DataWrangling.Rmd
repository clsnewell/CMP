...
FishIsotopeData<- RawIsotopeData_03302022 %>% filter(Taxon == "FISH")

FishIsotopeData$Fish_Number<- regmatches(FishIsotopeData$SampleID, gregexpr("[[:digit:]]+", FishIsotopeData$SampleID)) #makes a list.

FishIsotopeData$Fish_Number<-as.numeric(FishIsotopeData$Fish_Number)

FishIsotopeAndMetaData<-merge(FishIsotopeData, FishInfo_040122, by = "Fish_Number")

write.csv(FishIsotopeAndMetaData, "Data/FishIsotopeAndMetaData_040322.csv")

#Now let's follow the rest of the things I did in 2021...

#Remove Pilot fish and WL-Out... 
FishIsotopeAndMetaData_040322<- FishIsotopeAndMetaData_040322[!(FishIsotopeAndMetaData_040322$Site == "WL-O"),]
unique(FishIsotopeAndMetaData_040322$Site) #done

#Remove fish from pilot study
FishIsotopeAndMetaData_040322<- FishIsotopeAndMetaData_040322[!(FishIsotopeAndMetaData_040322$Season == "Pilot"),]
unique(FishIsotopeAndMetaData_040322$Season)

#Fix spelling errors
unique(FishIsotopeAndMetaData_040322$`Age Class`) #"Age 1+" & "age 1+", "adult" & "Adult"    
FishIsotopeAndMetaData_040322$`Age Class`<- as.character(FishIsotopeAndMetaData_040322$`Age Class`)
FishIsotopeAndMetaData_040322$`Age Class`[FishIsotopeAndMetaData_040322$`Age Class` == "age 1+"] <-"Age 1+"
unique(FishIsotopeAndMetaData_040322$`Age Class`)

FishIsotopeAndMetaData_040322$`Age Class`[FishIsotopeAndMetaData_040322$`Age Class` == "adult"] <-"Adult"

FishIsotopeAndMetaData_040322$`Age Class`[FishIsotopeAndMetaData_040322$`Age Class` == "juvenile"] <-"Juvenile"
unique(FishIsotopeAndMetaData_040322$`Age Class`)

#Need to split up muscle and liver data.

FishMuscleData<- FishIsotopeAndMetaData_040322[!(FishIsotopeAndMetaData_040322$Type_of_Material == "Liver"),]

FishLiverData<- FishIsotopeAndMetaData_040322[!(FishIsotopeAndMetaData_040322$Type_of_Material == "Muscle"),]
FishLiverData.<- FishLiverData[!(FishLiverData$Type_of_Material == "Muscle Tissue"),] 

#Now I want to combine sulfur and carbon/nitrogen data into single row.

glimpse(FishMuscleData)

FishMuscleData_Summarized<-FishMuscleData %>% select(Fish_Number, `D13CVPDB(permill)`, `D15NAir(permill)`, `D34SVCDT(permill)`, Site, Species, Field_Date, Season, `SL_(mm)`, `TL_(mm)`, `Age Class`)%>% group_by(Fish_Number) %>% summarise_all(list(~first(na.omit(.)))) #YES

FishLiverData_Summarized<-FishLiverData. %>% select(Fish_Number, `D13CVPDB(permill)`, `D15NAir(permill)`, `D34SVCDT(permill)`, Site, Species, Field_Date, Season, `SL_(mm)`, `TL_(mm)`, `Age Class`)%>% group_by(Fish_Number) %>% summarise_all(list(~first(na.omit(.)))) #YES
#Save it

write.csv(FishMuscleData_Summarized, "Data/FishMuscleDataSummarized_040422.csv")

write.csv(FishLiverData_Summarized, "Data/FishLiverDataSummarized_040422.csv")
```
...
## Now Pom and BA
```{r}
unique(RawIsotopeData_03302022$Taxon)
POMIsotopeData<- RawIsotopeData_03302022 %>% filter(Taxon == "POM/BA")
View(POMIsotopeData)
View(POM_BENTHIC_Info)
POM_BENTHIC_Info$SampleID<- POM_BENTHIC_Info$SIF_ID 
POM_BENTHIC_Info$SampleID <- gsub("_", "-", POM_BENTHIC_Info$SampleID) #Changes all the underscores to hyphens to match sif data.
POM_BA_IsotopeAndMetadata<-merge(POM_BENTHIC_Info, POMIsotopeData, by="SampleID")

write.csv(POM_BA_IsotopeAndMetadata, "Data/POM_BA_IsotopeAndMetaData_040422.csv")
